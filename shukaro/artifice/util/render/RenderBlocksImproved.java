...
public class RenderBlocksImproved
{
	...
	public boolean renderStandardBlock(RenderBlocks renderer, Block block, int x, int y, int z, int meta, boolean inverted)
	{
		...
		return Minecraft.isAmbientOcclusionEnabled() && Block.lightValue[block.blockID] == 0 ? (renderer.partialRenderBounds ? this.func_102027_b(renderer,
				block, x, y, z, f, f1, f2, meta, inverted) : this.renderStandardBlockWithAmbientOcclusion(renderer, block, x, y, z, f, f1, f2, meta, inverted))
				: this.renderStandardBlockWithColorMultiplier(renderer, block, x, y, z, f, f1, f2, meta, inverted);
	}
	...
	private boolean func_102027_b(RenderBlocks renderer, Block block, int x, int y, int z, float r, float g, float b, int meta, boolean inverted)
	{
		...
		if (frame != null)
		{
			...
			if (frame.textureRenderer != null)
			{
				...
				connectBottom = text.canConnectOnSide(renderer.blockAccess, new BlockCoord(x, y, z), 0, 0);
				connectTop = text.canConnectOnSide(renderer.blockAccess, new BlockCoord(x, y, z), 1, 0);
				connectNorth = text.canConnectOnSide(renderer.blockAccess, new BlockCoord(x, y, z), 2, 0);
				connectSouth = text.canConnectOnSide(renderer.blockAccess, new BlockCoord(x, y, z), 3, 0);
				connectWest = text.canConnectOnSide(renderer.blockAccess, new BlockCoord(x, y, z), 4, 0);
				connectEast = text.canConnectOnSide(renderer.blockAccess, new BlockCoord(x, y, z), 5, 0);
			}
		}
		...
		if(renderer.renderAllFaces || block.shouldSideBeRendered(renderer.blockAccess, x, y - 1, z, 0) && !connectBottom)
		{
		...
		if(renderer.renderAllFaces || block.shouldSideBeRendered(renderer.blockAccess, x, y + 1, z, 1) && !connectTop)
		{
		...
		if(renderer.renderAllFaces || block.shouldSideBeRendered(renderer.blockAccess, x, y, z - 1, 2) && !connectNorth)
		{
		...
		if(renderer.renderAllFaces || block.shouldSideBeRendered(renderer.blockAccess, x, y, z + 1, 3) && !connectSouth)
		{
		...
		if(renderer.renderAllFaces || block.shouldSideBeRendered(renderer.blockAccess, x - 1, y, z, 4) && !connectWest)
		{
		...
		if(renderer.renderAllFaces || block.shouldSideBeRendered(renderer.blockAccess, x + 1, y, z, 5) && !connectEast)
		{
	...
	private boolean renderStandardBlockWithAmbientOcclusion(RenderBlocks renderer, Block block, int x, int y, int z, float r, float g, float b, int meta, boolean inverted)
	{
		connectBottom = false;
		connectTop = false;
		connectEast = false;
	...
	private boolean renderStandardBlockWithColorMultiplier(RenderBlocks renderer, Block block, int x, int y, int z, float r, float g, float b, int meta, boolean inverted)
	{
		connectBottom = false;
		connectTop = false;
		connectEast = false;
