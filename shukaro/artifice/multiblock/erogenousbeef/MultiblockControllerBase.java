...
/**
 * This class contains the base logic for "multiblock controllers". You can
 * think of them as meta-TileEntities. They govern the logic for an associated
 * group of TileEntities.
 * 
...
public abstract class MultiblockControllerBase
{
    ...
    protected LinkedList<BlockCoord> connectedBlocks;
    
    /**
     * The coordinate of the block through which we communicate over the
     * network, and to/from which controllers save/load data. Generally, this
     * should also be the reference coordinate.
     */
     ...
     * if this is non-null, this coordinate will be checked for an IMachinePart
     * tile entity. If one exists, it will be added to this controller. This is
     * a trick to allow machines to create themselves during world loading
     * without all the blocks having to load their states at once.
     */
    ...
    /**
     * Set to true when adding/removing blocks to the controller. If true, the
     * controller will check to see if the machine should be
     * assembled/disassembled on the next tick.
     */
    ...
    protected MultiblockControllerBase(World world)
    {
        ...
        connectedBlocks = new LinkedList<BlockCoord>();
        saveDelegate = null;
        referenceCoord = null;
        cachedBlock = null;
        
        minimumCoord = new BlockCoord(0, 0, 0);
        maximumCoord = new BlockCoord(0, 0, 0);
     ...
     * 
     * @param initialBlock
     *            The coordinate of the block which is loading data into this
     *            controller.
     * @param savedData
     *            The NBT tag containing this controller's data.
     */
    public void loadAndCacheInitialBlock(BlockCoord initialBlock,
    ...
    /**
     * Call this to attach a block to this machine. Generally, you want to call
     * this when the block is added to the world.
     * 
     * @param part
     *            The part representing the block to attach to this machine.
     */
    public void attachBlock(IMultiblockPart part)
    {
        ...
        while (!partsToCheck.isEmpty())
        {
            ...
            if (!candidate.isConnected())
            {
                ...
                for (IMultiblockPart p : newParts)
                {
        ...
        if (this.saveDelegate == null)
        {
            ...
            TileEntity te = this.worldObj.getBlockTileEntity(saveDelegate.x,
                    saveDelegate.y, saveDelegate.z);
        ...
        else if (this.referenceCoord.compareTo(this.saveDelegate) < 0)
        {
            ...
            saveDelegate = referenceCoord;
            te = this.worldObj.getBlockTileEntity(saveDelegate.x,
                    saveDelegate.y, saveDelegate.z);
            ((IMultiblockPart) te).becomeMultiblockSaveDelegate();
        }
    }
    ...
    /**
     * Helper function. Check for orphans and create new machines until there
     * are no more orphaned blocks.
     */
    private void doFission()
    {
        ...
        for (BlockCoord c : connectedBlocks)
        {
            part = (IMultiblockPart) this.worldObj.getBlockTileEntity(c.x, c.y,
                    c.z);
        ...
        for (IMultiblockPart orphan : orphans)
        {
            ...
            {
                // Creating a new multiblock should capture all other orphans
                // connected to this orphan.
    ...
    protected boolean isMachineWhole()
    {
        if (connectedBlocks.size() >= getMinimumNumberOfBlocksForAssembledMachine())
        {
            ...
            {
                for (int y = minimumCoord.y; !dealbreaker
                        && y <= maximumCoord.y; y++)
                {
                    for (int z = minimumCoord.z; !dealbreaker
                            && z <= maximumCoord.z; z++)
                    {
                        ...
                        if (extremes >= 2)
                        {
                            if (part != null && !part.isGoodForFrame())
                            {
                                dealbreaker = true;
                                    ...
                                    && !isBlockGoodForFrame(this.worldObj, x,
                                            y, z))
                            {
                                dealbreaker = true;
                            }
                            ...
                            if (y == maximumCoord.y)
                            {
                                if (part != null && !part.isGoodForTop())
                                {
                                    dealbreaker = true;
                                        ...
                                        & !isBlockGoodForTop(this.worldObj, x,
                                                y, z))
                                {
                                    dealbreaker = true;
                                }
                            ...
                            else if (y == minimumCoord.y)
                            {
                                if (part != null && !part.isGoodForBottom())
                                {
                                    dealbreaker = true;
                                        ...
                                        & !isBlockGoodForBottom(this.worldObj,
                                                x, y, z))
                                {
                                    dealbreaker = true;
                                }
                            ...
                            else
                            {
                                ...
                                if (part != null && !part.isGoodForSides())
                                {
                                    dealbreaker = true;
                                        ...
                                        & !isBlockGoodForSides(this.worldObj,
                                                x, y, z))
                                {
                                    dealbreaker = true;
                                }
                        ...
                        else
                        {
                            if (part != null && !part.isGoodForInterior())
                            {
                                dealbreaker = true;
                                    ...
                                    & !isBlockGoodForInterior(this.worldObj, x,
                                            y, z))
                            {
                                dealbreaker = true;
                            }
    ...
    {
        TileEntity te;
        for (BlockCoord coord : connectedBlocks)
        {
     ...
     * being removed. Called onMachineBroken on all attached parts.
     */
    ...
    /**
     * Merge another controller into this controller. Acquire all of the other
     * controller's blocks and attach them to this machine.
     * 
    ...
    public void merge(MultiblockControllerBase other)
    {
        ...
        TileEntity te;
        List<BlockCoord> blocksToAcquire = (LinkedList<BlockCoord>) other.connectedBlocks
                .clone();
     ...
     */
    private void onMergedIntoOtherController(
            MultiblockControllerBase otherController)
    {
     ...
     * This means all blocks have been stripped out of this object and handed
     * over to the other controller.
     * 
     * @param otherMachine
     *            The machine consuming this controller.
     */
    protected abstract void onMachineMerge(MultiblockControllerBase otherMachine);
    
    /**
     * Called after all multiblock machine merges have been completed for this
     * machine.
     */
    ...
    // TODO: Change this so that people can safely override a business-logic
    // callback
    // and don't need to call super.
    /**
     * The update loop! Implement the game logic you would like to execute on
     * every world tick here. Note that this only executes on the server, so you
     * will need to send updates to the client manually.
     */
    ...
    /**
     * Recalculates the walk distance from the reference coordinate to all other
     * connected blocks.
     */
    protected void recalculateDistances()
    {
        ...
        for (BlockCoord c : connectedBlocks)
        {
            ...
            ((IMultiblockPart) te)
                    .setDistance(IMultiblockPart.INVALID_DISTANCE);
        ...
        IMultiblockPart part = (IMultiblockPart) this.worldObj
                .getBlockTileEntity(referenceCoord.x, referenceCoord.y,
                        referenceCoord.z);
        part.setDistance(0);
        ...
        while (!partsToCheck.isEmpty())
        {
            ...
            {
                assert (nearbyPart.getMultiblockController() == this);
                minimumNearbyDistance = Math.min(minimumNearbyDistance,
                        nearbyPart.getDistanceFromReferenceCoord());
        ...
        for (BlockCoord coord : connectedBlocks)
        {
            part = (IMultiblockPart) this.worldObj.getBlockTileEntity(coord.x,
                    coord.y, coord.z);
    ...
    private void recalculateMinMaxCoords()
    {
        ...
        for (BlockCoord coord : connectedBlocks)
        {
            if (coord.x < minimumCoord.x)
            {
                minimumCoord.x = coord.x;
            }
            if (coord.x > maximumCoord.x)
            {
                maximumCoord.x = coord.x;
            }
            if (coord.y < minimumCoord.y)
            {
                minimumCoord.y = coord.y;
            }
            if (coord.y > maximumCoord.y)
            {
                maximumCoord.y = coord.y;
            }
            if (coord.z < minimumCoord.z)
            {
                minimumCoord.z = coord.z;
            }
            if (coord.z > maximumCoord.z)
            {
                maximumCoord.z = coord.z;
            }
        }
    }
    ...
    protected void loadFromCachedBlock()
    {
        System.out.println(String.format(
                "Found cached block @ %d, %d, %d on %s", cachedBlock.x,
                cachedBlock.y, cachedBlock.z, this.worldObj.isRemote ? "client"
                        : "server"));
        TileEntity te = this.worldObj.getBlockTileEntity(cachedBlock.x,
                cachedBlock.y, cachedBlock.z);
        cachedBlock = null;
        assert (te instanceof IMultiblockPart);
        
        // This will spider out and reconnect to all the blocks loaded into the
        // world
        // on the first frame.
        this.attachBlock((IMultiblockPart) te);
        System.out.println(String.format(
                "Done rebuilding, %d blocks attached on %s",
                this.connectedBlocks.size(), this.worldObj.isRemote ? "client"
                        : "server"));
    }
}