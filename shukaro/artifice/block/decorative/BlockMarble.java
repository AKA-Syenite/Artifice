...
public class BlockMarble extends BlockArtifice
{
	...
	private Icon[] singleTextureList = new Icon[ArtificeCore.rocks.length];
	...
	{
		this.singleTextureList[0] = reg.registerIcon("artifice:marble");
		for (int i=1;i<ArtificeCore.rocks.length;i++)
		{
			if (i != 3 && i != 4)
			{
				String name = "artifice:marble_" + ArtificeCore.rocks[i].toLowerCase();
				this.singleTextureList[i] = reg.registerIcon(name);
			}
		}
		for (int j=0;j<47;j++)
		{
			String name = "artifice:marble/marble_paver_" + j;
