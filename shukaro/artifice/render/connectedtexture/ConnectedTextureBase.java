...
public abstract class ConnectedTextureBase
{
    ...
    public int getBlockTexture(IBlockAccess block, int x, int y, int z, int side)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (!canConnectOnSide(block, this.coord.set(x, y, z),
                    sideSideMap[side][i], side))
                map |= (7 << i * 2) % 256 | 7 >>> 8 - i * 2;
            else if ((!canConnectOnSide(block,
                    this.coord.set(x, y, z).offset(side0), side1, side))
                    || (!canConnectOnSide(block, this.coord.set(x, y, z)
                            .offset(side1), side0, side)))
                map |= 1 << i * 2;
        }
    ...
    public abstract boolean canConnectOnSide(IBlockAccess block,
            BlockCoord coord, int side, int face);
}
